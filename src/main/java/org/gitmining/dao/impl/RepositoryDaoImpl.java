...
public class RepositoryDaoImpl extends BaseDaoImpl implements RepositoryDao {
	...
	@Override
	public List<Repository> getSimpleReposByTagNameAndSortPagination(List<String> tag_name, Sort type, int begin,
			int itemsPerPage) {
		// TODO Auto-generated method stub
		Map<String, Object> map = new HashMap<String, Object>();
		...
		if (!tag_name.get(3).equals(""))
			map.put("key", tag_name.get(3));
		...
		if (type == Sort.GENERAL) {
			if (!tag_name.get(3).equals("")) {
				...
				return sqlSession.selectList("repo.getSimpleReposByTagNameSortGeneralPaginationAndKey", map);
			} else {
				return sqlSession.selectList("repo.getSimpleReposByTagNameSortGeneralPagination", map);
			}
		...
		if (!tag_name.get(3).equals("")) {
			...
			return sqlSession.selectList("repo.getSimpleReposByTagNameSortPaginationAndKey", map);
		} else {
			return sqlSession.selectList("repo.getSimpleReposByTagNameSortPagination", map);
		}
	}
	...
	public int getResultCountPagination(List<String> tagName, Sort type) {
		...
		if (!tagName.get(3).equals("")) {
			map.put("key", tagName.get(3));
			return sqlSession.selectList("repo.getSimpleReposByTagNameAndKey", map).size();
		} else
			return sqlSession.selectList("repo.getSimpleReposByTagName", map).size();
	}
	...
	public List<Languagetime> getLanguagetime() {
		// TODO Auto-generated method stub
		return sqlSession.selectList("repo.getLanguagetime");
	}

	@Override
	public List<ActionTime> getActiontime() {
		// TODO Auto-generated method stub
		return sqlSession.selectList("repo.getActiontime");
	}

	@Override
	public List<Duration> getDuration() {
		// TODO Auto-generated method stub
		return sqlSession.selectList("repo.getDuration");
	}

	@Override
	public List<LanguageContri> getLanguagecontri() {
		// TODO Auto-generated method stub
		return sqlSession.selectList("repo.getLanguagecontri");
	}

	@Override
	public List<Commit> getRecentCommit(String fn) {
		return sqlSession.selectList("repo.searchCommitByRepoName", fn);
	}

	@Override
	public List<Issue> getAllIssue(String fn) {
		return sqlSession.selectList("repo.searchIssueByRepoName", fn);
	}
}
