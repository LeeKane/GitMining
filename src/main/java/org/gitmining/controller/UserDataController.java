...
public class UserDataController {
	...
	@RequestMapping(value="/usercharts")
	public ModelAndView UserChart(HttpServletRequest request,HttpServletResponse response){
		...
		for(int i = 0;i < users.size();i++){
			String year = users.get(i).getCreated_at().split("-")[0];
...
//		result.put("companyCount", countList);
//
//		List<String> blogkeyList = new ArrayList<String>();
//		List<Integer> blogcountList = new ArrayList<Integer>();
//		Map<String, Integer> blogData = userDataService.getBlogUserCount();
//		Set<String> blogkeys = blogData.keySet();
//		for (String key : blogkeys) {
//			keyList.add(key);
//			int count = blogData.get(key);
//			countList.add(count);
//		}
//		for (int i = 0; i < blogcountList.size(); i++) {
//			for (int j = 0; j < blogcountList.size()-1; j++) {
//				if(blogcountList.get(j+1) > blogcountList.get(j)){
//					int tmp = blogcountList.get(j);
//					String ctmp = keyList.get(j);
//
//					countList.set(j, countList.get(j+1));
//					countList.set(j+1, tmp);
...
//		result.put("blogCount", countList);
		...
		return new ModelAndView("user_charts","result",result);
	}
}
