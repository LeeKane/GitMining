(function ($) {
  $(document).ready(function() {
    ...
    window.index.add({
      href: 'http://materializecss.com/user.html',
      title: 'Badges',
    ...
    $('input#search').bind('keyup', debounce(function (e) {
      ...
      var query = $(this).val();
      // var results2 = window.index.search(query).slice(0, 6).map(function (result) {
      //   var href = result.ref.split('http://materializecss.com/')[1];
      //   return [query + href.slice(1), 'http://materializecss.com/'+query];
      // });

      var repo=new Array(2);
      repo[0]="";
      repo[1]='/GitMining/repos?key='+query;
